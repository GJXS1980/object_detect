...
from std_msgs.msg import Int64, Int32MultiArray, String
...
class Findposition:
    def __init__(self):
        ...
        # 创建cv_bridge话题
        # 创建图像发布话题
        self.image_pub = rospy.Publisher("cv_bridge_image", Image)
        # self.image_pub1 = rospy.Publisher("test_image", Image, queue_size=1)


        # 创建识别物体中心坐标的发布话题
        self.pub = rospy.Publisher('object_center', object_result_msg, queue_size = 1)   
        
        rate = rospy.Rate(10) # 发布频率为10hz

        rospy.Subscriber("input_rgb_image", Image, self.color_detect)
    ...
    def color_detect(self,data):
        ...
        if color_judge:
            ...
            #   发布识别成功的信号
            if self.tag1 == 2 and self.center_ob != [0, 0] and self.side_judge:
                self.pub.publish(self.center_ob)
                #self.image_pub.publish(self.mask)
