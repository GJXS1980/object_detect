...
class Findposition:
    def __init__(self):
        ...
        #   发布消杀话题
        self.bw_pub = rospy.Publisher('ultraviolet_disinfection', Int32, queue_size=10)   

        while not rospy.is_shutdown():
            while(1):
                for i in range(0, 2):
                    self.get_color = i
                    self.getRobotInit_tag()
                    time.sleep(2)
                    if self.bw:
                        if self.center_ob[2] > 0 and i == 0:
                            playsound('/home/castlex/ws/src/object_detect/scripts/W.wav')
                            self.bw_pub.publish(0)
                          
                        elif self.center_ob[2] > 0 and i == 1:
                            playsound('/home/castlex/ws/src/object_detect/scripts/B.wav')
                            self.bw_pub.publish(1)
                        self.bw = False
                        self.center_ob = None
...
#####################      获取物体的中心坐标回调函数　　   ######################
    def image_callback(self, data):
        ...
        if len(points) == 2:
            ...
            # 边长判断
            self.side_judge = ((40 <= self.px and self.px <= 520) and (40 <= self.st[0] and self.st[0] <= 520) and (40 <= self.st[1] and self.st[1] <= 520))
	...
	# OpenCV4中findContours这个轮廓提取函数会返回两个值,①轮廓的点集(contours);②各层轮廓的索引(hierarchy)
        cnts, _ = cv2.findContours(img_cp, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        if len(cnts) == 0:
            img_boxpoints = (0, 0)
        else:
...
if __name__== '__main__' :
	...
	#rospy.spin()
        ...
        rospy.loginfo("object_detect  finished.")
