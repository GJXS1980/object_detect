...
class Findposition:
    def __init__(self):
        ...
        #   订阅语音识别的话题
        rospy.Subscriber("object_color", Int64, self.getColor)
        #   订阅机械臂初始化完成的话题
        ...
        #self.length_size = rospy.get_param('~side_length')
        rospy.spin()
    ...
    def image_callback(self, data):
        ...
        if len(points) == 2:
            ...
            if self.side_judge:
                ...
                if self.px == 0:
                    self.tagf = 1
                    print("not canter data")
                else:
                    #bili = self.px /self.length_size
                    #self.center_ob = [center_x/bili, center_y/bili, self.px]
                    print("--------")
                    #obj_center_point = [(320 - center_x)/bili, (240 - center_y)/bili]
                    #real_center_point = [obj_center_point[0], obj_center_point[1]-35, self.px]
                    self.center_ob = [center_x, center_y, self.px]
                    print("object_center_point is :", self.center_ob)
		    self.pub.publish(self.center_ob)
                    self.tag1 = 2
